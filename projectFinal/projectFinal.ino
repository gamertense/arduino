...
#define BUTTON A4 // We use Analogue pin 5 to read the light value from the LDR
...
boolean eyeZisactive = false;
int count = 0;

NewPing eyeR(t2, e2, maxDist ); // Initialise Ultrasonic sensor 2
NewPing eyeL(t1, e1, maxDist ); // Initialise Ultrasonic sensor 1
NewPing eyeR2(t2, e2, maxDist2 ); // Initialise Ultrasonic sensor 2
NewPing eyeL2(t1, e1, maxDist2 ); // Initialise Ultrasonic sensor 1
...
void loop()
{
  ...
  // read in the current light button level
  lightBtnAmount = analogRead( BUTTON );
  // make sure the light value is within the range of the Max Intensity of the displays
  lightBtnAmount = ( lightBtnAmount / 255 ) * MAX_INTENSITY;
  // set eye z boolean to true
  if (lightBtnAmount < 5)
    eyeZisactive = true;
  ...
  if ( nextBlink < millis() && eyeZisactive == false)
  {
  ...
  else if ( abs( difference ) < 250 || (distance1 == 0 && distance2 == 0 ))
  {
    ShowEye_Forward();
    currentState = 0;
  }
...
void ShowEye_Z()
{
  ...
  for (uint8_t row = 0; row < ROW_SIZE; row++)
  {
    ...
    mx.setColumn(row, eye_z[row]);
    // Set the colums of the display 2
    mx.setColumn(row + 8, eye_z[row]);
  }
}
